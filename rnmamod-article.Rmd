...
The response probability ratio [@Magder2003; @Turner2015] aligns better with a different approach for dealing with missing data, i.e., the  
selection model. This model distinguishes the participants based on their outcome and 
then further categorises them into those who completed the assigned 
intervention and those who left prematurely [@Little1995]. The success probability ratio [@Akl2013; 
@Turner2015], on the other hand, is more suited for use with the risk ratio, as it represents a ratio 
of risks. IMRoM is intuitively related to the ratio of means [@Mavridis2015]. 
Finally, IMOR and IMDoM are more commonly used in conjunction with the OR and 
the mean difference (MD) and standardised mean difference, respectively. In this 
article, we will only consider IMOR and IMDoM due to their intuitive relationship 
with the aforementioned effect measures which are also the most frequently employed in 
...
(log IMOR), can take values in ${\rm I\!R}$, where a value zero implies the missing-at-random 
assumption (ignorable missingness), while non-zero values indicate the missing-not-at-random assumption 
(non-ignorable missingness). Essentially, these informative 
missingness parameters quantify deviations from the missing-at-random assumption. 
As these parameters are typically unknown, analysts can consider one of the following 
situations: 
...
Typically, a normal distribution is assigned to both informative missingness parameters 
[@Higgins2008; @White2008; @Mavridis2015; @Turner2015; @Spineli2019; @Spineli2021]. 
In the Bayesian framework, analysts may assign a normal prior distribution on these 
parameters and can specify the mean and variance of the normal distribution to 
